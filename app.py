...
symptoms_list = list(symptoms_dict.keys())  # Get all 131 symptoms
selected_symptoms = st.multiselect("Select your symptoms:", symptoms_list)

# Initialize session state to store the prediction and details
...
if st.button("Predict"):
    if selected_symptoms:
        st.session_state['predicted_disease'] = get_predicted_value(selected_symptoms)
        st.session_state['desc'], st.session_state['pre'], st.session_state['med'], st.session_state['die'], st.session_state['wrkout'] = helper(st.session_state['predicted_disease'])
    else:
        st.write("Please select at least one symptom to predict the disease.")

# Buttons for displaying details
if st.session_state['predicted_disease']:
    if st.button("Predicted Disease"):
        st.subheader("Predicted Disease")
        st.write(st.session_state['predicted_disease'])
